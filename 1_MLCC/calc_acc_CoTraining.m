function [acc,precision,recall,f_measure,h_accuracy,ex_acc,label_acc] = calc_acc_CoTraining(truth,prediction)
     ...
     for i = 1:n_unlabelled
          same_labels = 0;
           ...
           ex_acc = ex_acc + (sum(prediction(i,:) == truth(i,:)) == n_labels);
            ...
            precision = precision + (same_labels/n_t_l);
            recall = recall + (same_labels/n_p_l);
