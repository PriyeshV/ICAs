...
for d = 1:size(datasets,2)
    ...
    for train_perc = train_percentage
       ...
       for k = 1:K
         ...
         cnt = 1;
         for test_id = test'
            ocnt(cnt,1) = length(intersect(intersect(find(links(1).adjmat(test_id,:)),train),intersect(find(links(2).adjmat(test_id,:)),train)));
            cnt = cnt + 1;
         end         
         ...
         bPosterPos = zeros(n_ids,n_labels);
         bPosterNeg = zeros(n_ids,n_labels);                
         ...
         for test_id = test'
             for label_id = 1:n_labels
                 for view_id = 1:n_views
                    ...
                    bPosterPos(test_id,label_id) = bPosterPos(test_id,label_id) + score(:,pos_index);
                    bPosterNeg(test_id,label_id) = bPosterNeg(test_id,label_id) + (1 - score(:,pos_index));
            ...
            bPosterPos(test_id,:) = bPosterPos(test_id,:) / n_views;
            bPosterNeg(test_id,:) = bPosterNeg(test_id,:) / n_views;
            if ML == 0
                [~,tid] = max(bPosterPos);
                predicted_labels(test_id,tid) = 1;
            else
                predicted_labels(test_id,:) = bPosterPos(test_id,:) >= bPosterNeg(test_id,:);
            end
         end
         bpredicted_labels = predicted_labels;
         %disp('Bootstrap Over');
             ...
             for test_id = test'
                ...
                for label_id = 1:n_labels                    
                    for view_id = 1:n_views
                        ...
                        view_link_features = [ view(test_id,:) base2_buildrelation(links(view_id).adjmat(test_id,:),predicted_labels)];         
                        ...
                        [~,score] = predict(views(view_id).classifier(label_id).model,view_link_features);                    
                ...
                if ML == 0
                    ...
                    p_label = PosterPos >= PosterNeg; 
                    for label_id = 1:n_labels
                        if p_label(label_id) ~= bpredicted_labels(test_id,label_id)
                            if p_label(label_id)
                                if  PosterPos(label_id) > bPosterPos(label_id)
                                    predicted_labels(test_id,label_id) = 1;
                                else
                                    predicted_labels(test_id,label_id) = 0;
                                end
                            else
                                if  PosterNeg(label_id) > bPosterNeg(label_id)
                                    predicted_labels(test_id,label_id) = 0;
                                else
                                    predicted_labels(test_id,label_id) = 1;
                                end
